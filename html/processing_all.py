...
def process_file(f):
    ...
    heading = soup.find("tr", class_="libraryTHY2_Center")
    for row in soup.find_all("tr", class_="dataTDRight"):
        row_data = row.find_all("td")
        if not row_data[1].text == "TOTAL":
            info["year"] = int(row_data[0].text)
            info["month"] = int(row_data[1].text)
            info["flights"] = {"domestic": int(row_data[2].text),
                               "international": int(row_data[3].text)}
            print(info)
            data.append(info)
    return data
